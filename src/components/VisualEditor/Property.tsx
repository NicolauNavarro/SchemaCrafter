...
export default function Property({
  ...
  return (
    ...
    >
      <motion.div
        layout
        className={`property-child absolute delay-300 left-56 -translate-x-12 pl-12 opacity-0 transition-all pointer-events-none ${
          name === Object.entries(objectSchema.properties ?? {})[0]?.[0]
            ? "-top-4"
            : "-translate-y-4 "
        }`}
      >
        <div className="w-80 p-2 py-4 rounded-xl bg-surface-light dark:bg-surface-dark text-sm flex flex-col relative actions-parent">
          <Actions
            schemas={schemas}
          ...
          >
            ...
            <p className="text-muted-light dark:text-muted-dark">
              {objectSchema.required?.find((r) => r == name) ? "true" : "false"}
            </p>
          </div>
...
function togglePropertyRequired({
  ...
  // 🚫 Don't toggle if it's marked as an ID
  if (targetProp.$comment?.startsWith("prisma:id")) {
    console.info(`Property "${targetKey}" is marked as ID and must remain required.`);
